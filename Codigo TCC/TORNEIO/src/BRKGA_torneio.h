...
#ifndef BRKGA_torneio_H
#define BRKGA_torneio_H
...
public:
	 ...
	 */
	BRKGA_torneio(unsigned n, unsigned p, double pe, double pm, double rhoe,
...
template< class Decoder, class RNG >
BRKGA_torneio< Decoder, RNG >::BRKGA_torneio(unsigned _n, unsigned _p, double _pe, double _pm, double _rhoe,
...
template< class Decoder, class RNG >
BRKGA_torneio< Decoder, RNG >::~BRKGA_torneio() {
	for(unsigned i = 0; i < K; ++i) { delete current[i]; delete previous[i]; }
}

template< class Decoder, class RNG >
const Population& BRKGA_torneio< Decoder, RNG >::getPopulation(unsigned k) const {
	return (*current[k]);
}

template< class Decoder, class RNG >
double BRKGA_torneio< Decoder, RNG >::getBestFitness() const {
...
template< class Decoder, class RNG >
const std::vector< double >& BRKGA_torneio< Decoder, RNG >::getBestChromosome() const {
	unsigned bestK = 0;
	for(unsigned i = 1; i < K; ++i) {
...
template< class Decoder, class RNG >
void BRKGA_torneio< Decoder, RNG >::reset() {
	for(unsigned i = 0; i < K; ++i) { initialize(i); }
}

template< class Decoder, class RNG >
void BRKGA_torneio< Decoder, RNG >::evolve(unsigned generations) {
...
template< class Decoder, class RNG >
void BRKGA_torneio< Decoder, RNG >::exchangeElite(unsigned M) {
...
template< class Decoder, class RNG >
inline void BRKGA_torneio< Decoder, RNG >::initialize(const unsigned i) {
...
template< class Decoder, class RNG >
inline void BRKGA_torneio< Decoder, RNG >::evolution(Population& curr, Population& next) {
...
/*------------------------------------Método de Seleção - TORNEIO --------------------------------*/
	...
	float  sorteioElite, sorteioNaoElite, fitnessElite, fitnessNaoElite, selecionadoElite, selecionadoNaoElite;
	int y = 0,k = 4, posicaoNaoElite, auxiliarElite, auxiliarNaoElite;
...
/*---------------------------Inserindo valores de Fitness---------------------------------------------------*/
	...
	for (int i = 0; i < p; ++i)
	{	
		vtNewFitness[i].first = i; // insere na matriz vtPerctFitness as posições
		vtNewFitness[i].second = curr.fitness[i].first; // insere na matriz vtPerctFitness os fitness
...
/*-----------------------Imprimindo a matriz toda com posição e fitness------------------------------*/
		...
		for (i = 0; i < p; ++i)
		{
			std::cout << "\nPosicao: "  << vtNewFitness[i].first ;
			std::cout << "\nFitness: "  << vtNewFitness[i].second;
		}
		*/
...
/*-----------------------------------Vetor Não Elite - Inserindo valores-------------------------------------*/	
	...
	for (y= pe; y < p; y++){
		...
		std::cout << "\n\nVetor Não Elite: "  << vtNaoElite[y].second;
...
/*---------------------------Armazena pai Elite--------------------------------------------------------*/
		...
		auxiliarElite = pe;
		//std::cout << "\n\n\n\n\nAuxiliar Elite: " << auxiliarElite;
		for (y = 0; y < k; y++)
		{	

			sorteioElite = rand( ) % pe;
			//std::cout << "\n\n\nSorteados Elite: " << sorteioElite;
		...
		//std::cout << "\n\nSelecionado Elite: "  << selecionadoElite;
...
/*---------------------------Armazena pai Não Elite--------------------------------------------------------*/
		...
		//std::cout << "\n\n\n\n\nAuxiliar Não Elite: " << auxiliarNaoElite;
		for (y = pe; y < pe + k ; y++)
		{	

			sorteioNaoElite = rand( ) % (p - pe) + pe;
		
			//std::cout << "\n\n\nSorteados Não Elite: " << sorteioNaoElite;
			...
			//std::cout << "\n\nPosição não Elite: "  << posicaoNaoElite;


			selecionadoNaoElite = vtNaoElite[posicaoNaoElite].second;
							
			//std::cout << "\n\nSelecionado Não Elite: "  << selecionadoNaoElite;


/*---------------------------FIM DO MÉTODO DE SELEÇÃO--------------------------------------------------------*/
		...
		const unsigned eliteParent = auxiliarElite;

		// Select a non-elite parent:
		const unsigned noneliteParent = auxiliarNaoElite;
...
template< class Decoder, class RNG >
unsigned BRKGA_torneio<Decoder, RNG>::getN() const { return n; }

template< class Decoder, class RNG >
unsigned BRKGA_torneio<Decoder, RNG>::getP() const { return p; }

template< class Decoder, class RNG >
unsigned BRKGA_torneio<Decoder, RNG>::getPe() const { return pe; }

template< class Decoder, class RNG >
unsigned BRKGA_torneio<Decoder, RNG>::getPm() const { return pm; }

template< class Decoder, class RNG >
unsigned BRKGA_torneio<Decoder, RNG>::getPo() const { return p - pe - pm; }

template< class Decoder, class RNG >
double BRKGA_torneio<Decoder, RNG>::getRhoe() const { return rhoe; }

template< class Decoder, class RNG >
unsigned BRKGA_torneio<Decoder, RNG>::getK() const { return K; }

template< class Decoder, class RNG >
unsigned BRKGA_torneio<Decoder, RNG>::getMAX_THREADS() const { return MAX_THREADS; }
