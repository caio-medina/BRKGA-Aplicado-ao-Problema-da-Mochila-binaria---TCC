...
#ifndef BRKGA_roleta_H
#define BRKGA_roleta_H
...
public:
	 ...
	 */
	BRKGA_roleta(unsigned n, unsigned p, double pe, double pm, double rhoe,
...
template< class Decoder, class RNG >
BRKGA_roleta< Decoder, RNG >::BRKGA_roleta(unsigned _n, unsigned _p, double _pe, double _pm, double _rhoe,
...
template< class Decoder, class RNG >
BRKGA_roleta< Decoder, RNG >::~BRKGA_roleta() {
	for(unsigned i = 0; i < K; ++i) { delete current[i]; delete previous[i]; }
}

template< class Decoder, class RNG >
const Population& BRKGA_roleta< Decoder, RNG >::getPopulation(unsigned k) const {
	return (*current[k]);
}

template< class Decoder, class RNG >
double BRKGA_roleta< Decoder, RNG >::getBestFitness() const {
...
template< class Decoder, class RNG >
const std::vector< double >& BRKGA_roleta< Decoder, RNG >::getBestChromosome() const {
	unsigned bestK = 0;
	for(unsigned i = 1; i < K; ++i) {
...
template< class Decoder, class RNG >
void BRKGA_roleta< Decoder, RNG >::reset() {
	for(unsigned i = 0; i < K; ++i) { initialize(i); }
}

template< class Decoder, class RNG >
void BRKGA_roleta< Decoder, RNG >::evolve(unsigned generations) {
...
template< class Decoder, class RNG >
void BRKGA_roleta< Decoder, RNG >::exchangeElite(unsigned M) {
...
template< class Decoder, class RNG >
inline void BRKGA_roleta< Decoder, RNG >::initialize(const unsigned i) {
...
template< class Decoder, class RNG >
inline void BRKGA_roleta< Decoder, RNG >::evolution(Population& curr, Population& next) {
...
/*----------------------------------Método de Seleção - ROLETA---------------------------------------------*/
	...
	double selecionadoElite, selecionadoNaoElite, somaElite, somaNaoElite;
...
/*---------------------------Inserindo valores de Fitness---------------------------------------------------*/
	...
	for ( y = 0; y < p; ++y)
	{	
		vtPerctFitness[y].first = y; // insere na matriz vtPerctFitness as posições
		vtPerctFitness[y].second = curr.fitness[y].first; // insere na matriz vtPerctFitness os fitness
		//somaFitness = somaFitness + vtPerctFitness[i].second; // soma os fitness
		
	}

/*-----------------------------------Vetor Elite - Inserindo valores-------------------------------------*/

	/*Pegando valores de Elite do vetor PerctFiness e inserindo valores no vetor Elite */
	...
	*/
	...
	for (y=0; y < pe; y++){
		...
		std::cout << "\n\nPorcentagem Vetor Elite: "  << vtElite[y].second;
...
/*-----------------------------------Vetor Não Elite - Inserindo valores-------------------------------------*/	
	...
	/*Pegando valores de Não Elite do vetor PerctFiness e inserindo valores no vetor Não Elite */
	...
	for (y= pe; y < p; y++){
		...
		std::cout << "\n\nVetor Não Elite ANTES: "  << vtNaoElite[y].second;
	...
	*/
	...
	for (y = pe; y < p; ++y){
			...
			vtNaoElite[y].second = (vtNaoElite[y].second)/somaNaoElite; 
	...
	for (y = pe; y < p; y++){
		...
		std::cout << "\n\nVetor Não Elite Depois: "  << vtNaoElite[y].second;
...
/*------------------------------------GIRA A ROLETA NO ELITE--------------------------------------------*/
	...
	//std::cout << "\n\nPai Elite selecionado: "  << vtElite[identElite].second;
...
/*-----------------------------------GIRA A ROLETA NO NÃO ELITE--------------------------------------------*/
	...
	while(selecionadoNaoElite < sorteioNaoElite)
	{
		...
		identNaoElite++;
		selecionadoNaoElite = selecionadoNaoElite + vtNaoElite[identNaoElite].second;
		
	}
	
	
	//std::cout << "\n\nPai Nao Elite selecionado: "  << vtNaoElite[identNaoElite].second;
...
/*--------------------------------------------IMPRESSÕES----------------------------------------------------*/
	...
	//std::cout << "\n\nFitness Elite selecionado: "  << selecionadoElite; 
	
	//std::cout << "\n\nFitness Nao Elite selecionado: "  << selecionadoNaoElite;

	//std::cout << "\n\nSoma Elite: "  << somaElite;

	//std::cout << "\n\nSoma Nao Elite: "  << somaNaoElite;
...
// 3. We'll mate 'p - pe - pm' pairs; initially, i = pe, so we need to iterate until i < p - pm:
		...
		const unsigned eliteParent = identElite;

		// Select a non-elite parent:
		const unsigned noneliteParent = identNaoElite;
...
template< class Decoder, class RNG >
unsigned BRKGA_roleta<Decoder, RNG>::getN() const { return n; }

template< class Decoder, class RNG >
unsigned BRKGA_roleta<Decoder, RNG>::getP() const { return p; }

template< class Decoder, class RNG >
unsigned BRKGA_roleta<Decoder, RNG>::getPe() const { return pe; }

template< class Decoder, class RNG >
unsigned BRKGA_roleta<Decoder, RNG>::getPm() const { return pm; }

template< class Decoder, class RNG >
unsigned BRKGA_roleta<Decoder, RNG>::getPo() const { return p - pe - pm; }

template< class Decoder, class RNG >
double BRKGA_roleta<Decoder, RNG>::getRhoe() const { return rhoe; }

template< class Decoder, class RNG >
unsigned BRKGA_roleta<Decoder, RNG>::getK() const { return K; }

template< class Decoder, class RNG >
unsigned BRKGA_roleta<Decoder, RNG>::getMAX_THREADS() const { return MAX_THREADS; }
